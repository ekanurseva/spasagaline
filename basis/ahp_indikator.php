...
$kriteria_nama = query("SELECT nama_kriteria FROM kriteria WHERE idkriteria = $idkriteria")[0];
$data_ind = jumlah_data("SELECT * FROM ind_gejala WHERE idkriteria = $idkriteria");
$indikator = query("SELECT * FROM ind_gejala WHERE idkriteria = $idkriteria");

$enkripsi = enkripsi($idkriteria);

$ind = query("SELECT * FROM ind_gejala WHERE idkriteria = $idkriteria ORDER BY idindikator ASC");

$a = $data_ind * $data_ind;
$data = jumlah_data("SELECT * FROM rel_indikator WHERE idkriteria = $idkriteria");
if ($data < $a) {
    input_kode_indikator($idkriteria);
}

$rel_ind = query("SELECT * FROM rel_indikator WHERE idkriteria = $idkriteria");
...
if (isset($_POST['submit_ubah'])) {
    ...
    $data_kode[] = $data_kode1['kode_indikator'];
    $data_kode[] = $data_kode2['kode_indikator'];
    ...
    for ($i = 0; $i < count($data_kode); $i++) {
        for ($j = 0; $j < count($data_kode); $j++) {
            ...
            $data_nilai = query("SELECT * FROM rel_indikator WHERE kode1 = '$kode1' AND kode2 = '$kode2'")[0];
    ...
    <div class="main-container d-flex">
        ...
        <div class="content">
            ...
            <div class="contents px-4 py-3">
                <h4 class="text-white text-center pb-3">ANALISA HIERARKI INDIKATOR</h4>

                <div class="tabel text-white px-5 py-4">
                    <div class="row">
                        <div class="col-9">
                            <input class="form-control mb-2" type="text" name="kriteria"
                                value="<?= $kriteria_nama["nama_kriteria"]; ?>" readonly>
                        </div>
                        <div class="col-3 p-0 d-flex justify-content-center">
                            <div class="ahp_btn ">
                                <span style="font-size: 9px;">Klik disini untuk mulai analisis CF
                                    Pakar </span>
                                <a href="hasil_ahp.php?id=<?= $enkripsi; ?>"
                                    style="padding: 0 45px; text-decoration: none;" type="submit" name="aksi">
                                    ANALISIS
                                </a>
                            </div>
                        </div>
                                    ...
                                    <thead>
                                        <tr>
                                            ...
                                            <td class="fw-medium" scope="col">
                                                <?= $data_kode1['kode_indikator']; ?>
                                            </td>
                                            <td class="fw-medium" scope="col">
                                                <?= $data_kode2['kode_indikator']; ?>
                                            </td>
                                        </tr>
                                    </thead>
                                    <tbody>
